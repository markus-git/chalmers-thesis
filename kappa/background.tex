...
\noindent where \codei{Exp} is a short-hand for expressions and is defined as a type synonym for integers thus an example of a shallow EDSL.

The language around \codei{Exp} is defined by two functions: one for integer literals, called \codei{const}; and another for multiplication, called \codei{times}. Note we could easily have added more functions, as long as they're integral expressions. Another advantage of \codei{Exp} is that we can quickly calculate its value:
