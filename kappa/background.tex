...
While the size of the intermediate result of \codei{f} could potentially spoil any usefulness of the composition, functional programming solves this by only evaluating \codei{f} as much as is needed by \codei{g}. This property is referred to as lazy evaluation and lets us fuse functions without creating any unnecessary, intermediate values. In fact, its benefits can extend to embedded types as well, like vectors.
...
Two benefits of a shallowly embedded language is that its easy to add new functions, and evaluation is simply the identity funciton. ..
...
\noindent where \codei{Exp} is now a datatype that lists all supported expressions, which \codei{const} and \codei{times} use construct their results.
...
As \codei{const} and \codei{times} return a representation of their result rather than the result itself, it is hardware to add new functions without first extending the \codei{Exp} datatype. The syntax tree used for a deeply embedded type like \codei{Exp}, while inflexible compared to its shallow version, enables interpretation of expressions in order to, for instance, evaluate them:
...
While the implementation of shallow and deep embedding are usually at odds, there has been work done in order to combine their benefits~\cite{svenningsson2012}. ...
