...
\noindent \codei{fmap} here is similar to the above \codei{map} but works for vectors instead of lists, \codei{zip} joins two vectors, and \codei{fold} reducs a vector into a scalar value using addition and starting at zero. These three functions are all part of the vector library.
...
\noindent A program in the co-design languages is a monad: a kind of composable computation descriptions. Together with Haskell's \codei{do} notation, a monad allow instructions to be sequenced in much the same way as in C.
