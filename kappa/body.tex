...
In Haskell, functions like the dot-product can be implemented in a compositional style using two operations from its standard libraries:
...
\noindent Here, \codei{zipWith} combines the two input lists by element wise multiplication, but only going as far as the shortest of the two lists, and \codei{sum} then sums together the resulting list. The type is also generalized from the C variant, as it accepts any numerical value (given by the \codei{Num} constraint on \codei{a}) instead of just doubles.
