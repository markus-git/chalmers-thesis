...
Functional programming, like its name implies, is based around the application of functions to its arguments. Programs are written as functions that accept input and delivers its result. These functions themselves are defined in terms of smaller functions, which in turn are defined used smaller functions still. At the very end, a function will only consist of language primitives, like built-in functions, variables or constants. An interesting distinction between functions in a functional language and, say an imperative language like C, is that functions always return the same value when given same arguments. This comes as a consequence from how functional languages contain no assignment statements, so a variable will never change once it has been given a value. More generally, functional programs have no side effects. This property turns out to be quite handy, as it makes the separates the function definition from its evaluation. Functions can be evaluate in parallel as long at their data dependencies are satisfied.

A function that accepts other functions as arguments is often referred to as a higher-order function, or a combinator, and provides the aforementioned glue code that lets programmers build functions from smaller ones. In Haskell, a functional programming language, a number of such higher-order functions are provided that implements common operations. One such function is the \codei{map} function, which can be defined as follows.
...
The first line specifies the type of the function, and says that its first argument is a function \codei{f :: a -> b} that, given an argument of type \codei{a}, produces a result of type \codei{b}. In addition to the function, \codei{map} takes a list \codei{xs :: [a]} where each element have type \codei{a}, and returns another list over elements with type \codei{b}. As functions have no side effects, we can already guess what this function does: it applies the function \codei{f} to every element of a list and returns the resulting list. Looking at the second and third line, we see that this is actually what the function does. The second line states that applying the function to an empty list returns another empty list, while the third line states that, for a non-empty list, its function is applied to the element and then recursively calls itself on the reminder. In Haskell, \codei{[]} is an empty list and \codei{x:xs} is a non-empty list with the element \codei{x} as its head and the elements \codei{xs} as its tail.

A function like \codei{map} operates over any type \codei{a} and \codei{b}, and has no way of inspecting what these types actually are. They could be primitive values like integers, but they could just as well be other functions that we compose with \codei{f}, a program that is repeatedly applied to a number of inputs, or 
