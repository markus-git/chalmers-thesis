...
\begin{code}
firH :: (Num a, HType a) => Int -> HVArr a -> Signal a
     -> Hardware (Signal a)
\end{code}
...
\noindent \todo{Note that, the arrays used for taps is held by a signal, while the array type used for coefficients uses a variable.}

\todo{Connecting the hardware filter to a main program body that supplies it with its inputs and compiling it produces the following VHDL design:}
...
\noindent Note that the type constraint is slightly different from previous functions, as it makes us of \codei{Type} instead of \codei{TypeM}. The former type constraint is designed for functions are parameterized on their expression type, while the latter is used with monads.
