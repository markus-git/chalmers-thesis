...
Being able to implement both \codei{reverseS} and \codei{reverseH} using the same functions hints that us marking them as software and hardware specific programs was unnecessary. We would be better of keeping the reverse function generic, enabling it do be used in both languages. After all, the function only require that its implementer supports the for-loop and array operations. This kind of language agnostic function is supported by the co-design library through its use of Haskell's type classes, which enables us to put additional constraints on which operations a function needs in order to operate. In the case of our reverse function, we could give it the following type:
...
\noindent Here, the previous array types of \codei{SArr} and \codei{HArr} have been substituted for the more general type of \codei{Arr}, which is parameterized on the monad \codei{m} that will eventually be instantiated as either software or hardware. The general idea behind \codei{Arr} is that its type will turn into a \codei{SArr} or \codei{HArr} type when \codei{m} is instantiated with their corresponding language.

Our new and generic function for reversing an array has introduced two new types from our co-design library, namely the \codei{Arrays} and \codei{Control} constraints. These two ensure that whichever monad the function gets eventually gets instantiated as will support the necessary array and control operations:
