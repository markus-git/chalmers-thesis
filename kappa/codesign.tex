...
\codei{dist} computes the distance between two points in a plane, where points are represented as a pair of coordinates. The pairs are however syntactic suger, and no pairs will appear in the generated code.
...
So far the functions we shown are combinatorial. Most interesting examples of signal processing does however carry some form of state. For this reason, a sequential operator is also provided by the signal language:
...
While these functions may appear innocent, \codei{repeat}, \codei{map} and \codei{zipWith} can express most combinatorial signal networks, while the combination of \codei{delay} with feedback can describe any kind of sequential signal network. As an example, we implement a infinite impulse response (IIR) filter, which comprises the second primary type of digital filters used in digital signal processing applications and, unlike FIR filter in section~\ref{embedded}, contains feedback.
...
\noindent $P$ and $Q$ are the feed-forward and feedback filter orders and $a_{j}$ and $b_{i}$ are the filter coefficients.

Examining the above equation we see that a IIR filter loosely consists of two FIR filters, where the bottom FIR filter has an extra delay and is recursively defined on its output. As such, we can define the filter as:
