...
The co-design language is intended to provide a convenient model of imperative programs and, as section~\ref{embedded} showed, also serves as base that extensions like the vector and signal processing languages can be build upon. Furthermore, the ability to write generic programs facilitates design exploration, and while they are not always ideal as hardware description---purely computational instructions does not support, for instance, pipelining---once a layout has been decided, it is easy to fix a program's type to hardware and optimize its implementation.
...
Instructions are parameterized on the type of their subprograms to facilitate an compositional definition of them that works for both simple instructions and control instructions, using a technique like Data Types \`{a} La Carte~\cite{DTC}. Both \codei{Interp} and \codei{HFunctor} can in fact be instantiated for the product of any instructions that in turn support their respective class, which means that new instructions can be defined and given an interpretation without worrying about whatever instruction set they are part of. Instructions that can be extended in this way are quite useful to the co-design language, as each computational element usually come with its own set of operations.
...
\noindent where \codei{P3} is synonym for a type-level list of three arguments, here \codei{prog}, \codei{exp}, and \codei{pred}. Although \codei{pred} is not used by \codei{If}, it is included to keep its type consistent with other instructions that do use it. Making \codei{If} an instance of the \codei{HFunctor} class is straightforward, as it simply calls applies the function on its two subprograms.

TODO.

% A program type with its , \codei{If} can be added its set of instructions like so:

\begin{code}
type MyProgram = Program (Old :+: If) (P2 Exp Pred)
\end{code}

\noindent The program type will set the type of subprograms in \codei{If} to \codei{MyProgram}, and its expression and predicate types to \codei{Exp} and \codei{Pred}.

If the old instrucion set of \codei{MyProgram} supported, for instance, interpretation in the code generation monad \codei{CGen}, the extended version no longer does: \codei{If} has yet been given such an intrepretation. Assuming \codei{Exp} can be interpreted in the \codei{CGen} monad, TODO.
...
\noindent Note that the resulting memory pointer of \codei{mmap} has ``softened'' its component's original type. Softening refers to the process of replacing any hardware specific types, like signals, with their corresponding type in software, and is described by the type family \codei{Soften}:
...
\noindent Where \codei{inputArr} adds an array of a given length to the signature, and \codei{returnC} rounds of the type signature with a signal for the dot product's output---the name eludes to a combinatorial return value, rather than it returning a value for the C language. The wrapped \codei{dot} can be hooked up to an interconnect with \codei{axi4lite} and then be compiled, synthesized and put onto hardware.
...
Manifest vectors are more often than not the immutable arrays provided by the co-design language, as such arrays have an representation in memory. A \codei{Pull} vector on the other hand is not stored in memory, but is rather represented as a function:
...
Pushy vectors go in the opposite direction of pull vectors, and gives us control over a vectors evaluation to the producers rather than the consumer. That is, pushy vectors have a representation that supports nested writes to memory and fusion of operations. A \codei{Push} vector is represented as:
