...
\noindent where \codei{P3} is synonym for a type-level list of three arguments, here \codei{prog}, \codei{exp}, and \codei{pred}. Although \codei{pred} is not used by \codei{If}, it is included to keep its type consistent with other instructions that do use it. \codei{If} can already be added to a program's set of instructions like so:

\begin{code}
type MyProgram = Program (Old :+: If) (P2 Exp Pred)
\end{code}

\noindent Here, \codei{Old}, \codei{Exp}, and \codei{Pred} are some instruction, expression, and predicate types. The recursive program type will set the type of subprograms in \codei{If} to \codei{MyProgram}.
