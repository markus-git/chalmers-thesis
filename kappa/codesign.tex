...
\noindent Normally we cannot inspect the function's type from within Haskell, so before we can hook up the function to an AXI4-lite interconnect we need to give it a type signature that we can read:
...
\noindent Where \codei{inputArr} adds a size array to the signature and \codei{returnC} rounds of the signature with the dot product. 
