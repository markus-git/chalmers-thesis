...
\noindent \codei{repeat} creates a signal by repeating some value; \codei{map} applies a function to each value of a signal; \codei{zipWith} joins two signals element-wise using then given function. The general idea is that every ground type in the expression language is lifted to operate element-wise over signals by, for example, using the above functions or type classes like \codei{Num}:
...
\noindent With the FIR filter defined as.

\begin{code}
fir :: (Fractional a, Num (exp a), pred a) => [exp a]
  -> Sig exp pred a -> Sig exp pred a
fir as x = P.foldr1 (+)
         $ P.zipWith (*) (P.map repeat as)
         $ P.iterate (delay a) x
\end{code}

\noindent Where functions prefixed by \codei{P} are Haskell's standard list function rather signal functions.
