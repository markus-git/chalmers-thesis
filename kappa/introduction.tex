...
References, arrays, and for-loops are supported in both C and hardware descriptions. As such, \codei{dotVec} is not tied to either software or hardware and can in fact be compiled to both. Most interesting programs for heterogeneous systems do however make use of a mixture of software and hardware designs. As an example, assume \codei{dotVec} should be mapped onto hardware and that a software program should to call it and print its result to standard output. Before \codei{dotVec} can be offloaded however, it needs to be given a signature over its input and output channels:
...
With a description of a hardware component like \codei{comp}, the compiler can automatically connect it to an AXI4-lite interconnect---a standard bus interface---that is ready for synthesis. Its possible to reach such components from software through, for example, memory-mapped I/O. The general idea is that a memory-mapped hardware component will share its address space with the software program, and can therefore be called like a regular function:
