...
% Outside of the Lava family, there is C$\lambda$ash~\cite{baaij2010}, a functional hardware description language that is embedded in Haskell. C$\lambda$ash borrows both its syntax and semantics from Haskell to provide a structural design approach that would be familiar to functional programmers, and provides a compiler that is able to produce low-level synthesizable code.
...
% , and its library enables a designer to simulate concurrent processes. SystemC does however have a bit of a syntactical overheard when compared to VHDL and Verilog when used as a hardware description language, and is perhaps better described as a language for system-level modeling.
