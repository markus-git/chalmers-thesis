...
In this thesis we explore a functional approach to heterogeneous system development, with a staged hardware software co-design language embedded in Haskell, to address many of the modularity problems typically found in such systems. This staged approach enables designers to build their applications from reusable components and skeletons, while retaining control over much of the generated source code. Design exploration can also benefit from a functional approach, as Haskell's powerful type system allow for fine grained control over the dependencies of a design. For instance, the co-design language can support not only software and hardware specific functions but also language independent functions that only rely on the functions they use.

The co-design language is based on a monadic representation of imperative programs that is loosely coupled to its statement, expression, and predicate types. This makes it easy to support extensible types, that is, the current language can be adapted to new processing elements with their own intrinsic operations, and to introduce new interpretations. The model and its associated techniques are not limited to hardware software co-design, and could be of use for other imperative languages as well.

In addition to the hardware software co-design, this thesis also introduces a signal processing and vector language. The first extends an expression type with support for synchronous data-flow computations, while the second extends an array type with a functional representation of vectors that support fusion. Both extensions can be compiled to the co-design language, but neither extension is dependent on it and can be fitted for use in other embedded languages.
}
